...
def plastico_bingham_int(gasto, diametro_interior, densidad_lodo, longitud, punto_cedencia, visc_plastica):
    ...
    vel_critica = ((7.75 * visc_plastica) + 7.75 * math.sqrt((visc_plastica ** 2) +
                                                              ...
                                                              * densidad_lodo))) / (densidad_lodo * diametro_interior)
    if vel_flujo < vel_critica:
        """Laminar"""
        return ((visc_plastica * longitud * vel_flujo) / (389081 * dimetro_cuadrado)) + ((punto_cedencia * longitud) /
...
def plastico_bingham_ea(gasto, diametro_mayor, diametro_menor, densidad_lodo, longitud, punto_cedencia, visc_plastica):
