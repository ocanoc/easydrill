...
class LeyDePotencias:
    ...
    def interior(fluido, lec_fan_300, lec_fan_600, gasto, diametro_interior, densidad_lodo, longitud,
        ...
        nre = (densidad_lodo * math.pow(vel_flujo, 2) / (2.319 * indice_consistencia)) * math.pow(
    ...
    def espacion_anular(fluido, lec_fan_300, lec_fan_600, gasto, diametro_mayor, diametro_menor, densidad_lodo,
                        longitud, punto_cedencia, visco_plastica):
        ...
        nre = (densidad_lodo * math.pow(vel_flujo, 2) / (1.65 * indice_consistencia)) \
              * pow(((1.25 * ea * n) / (vel_flujo * ((2 * n) + 1))), n)
        fluido.set_k(indice_consistencia)
        fluido.set_n(n)
        nre_tl = 3470 - (1370 * n)
        nre_tt = 4270 - (1370 * n)
        ...
        if nre <= nre_tl:
            ...
            return (f * densidad_lodo * math.pow(vel_flujo, 2) * longitud) / (48251 * ea)
